    ...
    protected function generateUniqueToken(Request $request)
{
    ...
    // Получаем последний токен с таким же префиксом
    $lastToken = QrCodes::where('token', 'like', "$prefix%")
        ->orderBy('token', 'desc')
        ->value('token');

    // Определяем стартовый номер
    $lastNumber = $lastToken ? (int)substr($lastToken, -3) : 0;
    ...
    for ($i = 0; $i < $quantity; $i++) {
        $lastNumber++;
        if ($lastNumber > 999) {
            return redirect()->back()->with('error', 'Достигнут предел генерации токенов (999).');
        }

        $token = $prefix . str_pad($lastNumber, 3, '0', STR_PAD_LEFT);
        $usedTokens[] = $token;
        $generatedTokens[] = [
            'token' => $token,
            'created_at' => now(),
